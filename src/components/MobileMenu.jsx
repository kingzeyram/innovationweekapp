import { useEffect } from "react"

export const MobileMenu = ({ menuOpen, setMenuOpen }) => {


    return (
        <div className={`fixed top-0 left-0 w-full bg-[rgba(10,10,10,0.8)] z-40 flex flex-col items-center justify-center
       transition-all duration-300 ease-in-out ${menuOpen ? "h-screen opacity-100 pointer-events-auto z-50" : "h-0 opacity-0 pointer-events-none"} `}>

            <button onClick={() => setMenuOpen(false)}
                className="absolute top-6 right-6 text-white text-xl focus:outline-none cursor-pointer"
                aria-label="Close Menu">
                &times;
            </button>

            <a href="#home"
                onClick={() => setMenuOpen(false)}
                className={`text-2xl font-semibold text-white hover:text-yellow-500 my-4 transform-transition-transform duration-300
            ${menuOpen ? "opacity-100 translate y-0" : "opacity-o translate y-5"}
            `}
            >Home</a>

            <a href="#highlight"
                onClick={() => setMenuOpen(false)}
                className={`text-2xl font-semibold text-white hover:text-yellow-500 my-4 transform-transition-transform duration-300
            ${menuOpen ? "opacity-100 translate y-0" : "opacity-o translate y-5"}
            `}
            >Highlight</a>

            <a href="#programs"
                onClick={() => setMenuOpen(false)}
                className={`text-2xl font-semibold text-white hover:text-yellow-500 my-4 transform-transition-transform duration-300
            ${menuOpen ? "opacity-100 translate y-0" : "opacity-o translate y-5"}
            `}
            >Programs</a>

            <a href="#speakerbios"
                onClick={() => setMenuOpen(false)}
                className={`text-2xl font-semibold text-white hover:text-yellow-500 my-4 transform-transition-transform duration-300
            ${menuOpen ? "opacity-100 translate y-0" : "opacity-o translate y-5"}
            `}
            >Speakers</a>

        </div>
    );
};